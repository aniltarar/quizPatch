    ...
    return (
        <div className='w-full  h-screen p-5 flex flex-col gap-y-3'>
            ...
            <div className='w-full flex justify-start items-center gap-x-5'>
                <LeftSide examInfo={examInfo} setExamInfo={setExamInfo} examQuestions={examQuestions} setExamQuestions={setExamQuestions} />
                        ...
                        examQuestions.map((question) => (
                            <div key={question.id} className='p-3 rounded-md bg-zinc-50 text-zinc-700 border    '>
                                <div className="flex justify-between items-center pb-3">
                                    <h1 className='pb-2 font-medium text-lg'>{question.examQuestionName}</h1>
                                    <button onClick={() => deleteQuestionState(question.id)} className='bg-red-500 px-4 py-2 rounded-md text-white border-red-600'>Soruyu Sil</button>
                                </div>
